                    ...
                    <form action="admin.php?action=editTheme" method="post">
                        <div class="card-body" id="changeTheme">
                            <div class="alert alert-success">
                                <strong>Si vous souhaitez modifier un thème, modifiez une copie de l'original en créant
                                    un nouveau thème. Cela vous évitera de perdre votre thème lors d'une mise à jour
                                    !</strong>
                            </div>
                            <label class="control-label" for="theme">
                                <a data-toggle="popover" data-trigger="hover" title="Besoin d'un nouveau thème ?"
                                   data-content="Découvrez et Télécharger un thème sur la <a href='https://craftmywebsite.fr/forum/index.php?resources/featured' target='_blank'>page officiel de craftmywebsite</a>!">Thèmes<i
                                            class="fas fa-info-circle"></i></a>
                            </label>
                            <select class="form-control text-center" name="theme" id="theme">
                                <option value="<?php echo $_Serveur_['General']['theme']; ?>"
                                        selected><?php echo $_Serveur_['General']['theme']; ?></option>
                                <?php if (isset($themes)) {
                                    foreach ($themes as $element) {
                                        if ($element != 'upload' and $element != $_Serveur_['General']['theme'] and $element != '..') { ?>
                                            <option
                                            value="<?php echo $element; ?>"><?php echo $element; ?></option><?php }
                                    }
                                } ?>
                            </select>
                        </div>
                    ...
                    <form method="POST" action="?&action=postBG" enctype="multipart/form-data">
                        <div class="card-body">
                            <div class="alert alert-warning">
                                <strong>Attention, tous les thèmes ne prennent pas en charge cette
                                    fonctionnalité.</strong>
                            </div>
                            <div class="alert alert-success">
                                <strong>Si vous voulez mettre une image, l'image doit être en 1920*1080 minimum pour
                                    s'adapter aux écrans et pour avoir une qualité optimale ! Vous pouvez aussi mettre
                                    une petite image qui se répètera !</strong>
                            </div>
