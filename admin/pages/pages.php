...
<?php if (!$_Permission_->verifPerm('PermsPanel', 'pages', 'showPage')) {
    ...
    <div class="alert alert-success">
        <strong>En plus des pages de base du CMS, vous pouvez créer des pages personnalisées. Vous pouvez utiliser ces
            ...
            <div class="col-md-12 col-xl-6 col-12">
                <div class="card  ">
                    ...
                    <div class="card-body" id="addPage">
                        ...
                        <textarea data-UUID="PHP0005" id="ckeditor" name="content"
                                  style="height: 275px; margin: 0px; width: 50%;"></textarea>
                    </div>

                    <script>initPost("addPage", "admin.php?&action=creerPage", function (data) {
                            if (data) {
                                clearAllInput("addPage");
                                pagesUpdate();
                            }
                        });</script>
                    ...
                    <div class="card-footer">
                        <div class="row text-center">
                            <input type="submit" onclick="sendPost('addPage', null, true);"
                            ...
                            <ul class="nav nav-tabs">
                                <?php for ($i = 0; $i < count($pages); $i++) { ?>
                                    <li class="nav-item" id="tabpage-<?php echo $i; ?>"><a
                                                ...
                                                id="pagetab-<?php echo $i; ?>"><?php echo $pages[$i]['titre']; ?></a>
                                    </li>
                                <?php } ?>
                            </ul>
                                    ...
                                    <div class="tab-pane well <?php if ($i == 0) echo 'active'; ?>"
                                        ...
                                        <div style="width: 100%;display: inline-block">
                                            ...
                                            <div class="float-right">
                                                <button onclick="sendDirectPost('admin.php?action=supprPage&name=<?php echo $pages[$i]['titre']; ?>', function(data, otherdata) { if(data && jsonDataIsOk(otherdata)) { hide('tabpage-<?php echo $i; ?>'); hide('page<?php echo $i; ?>'); } }, true);"
                                        ...
                                        <input type="text" id="titre<?php echo $i; ?>" class="form-control"
                                               onkeyup="get('pageTitle-<?php echo $i; ?>').innerText = get('pagetab-<?php echo $i; ?>').innerText = this.value; "
                                               name="titre" value="<?php echo $pages[$i]['titre']; ?>" required/>

                                        <input type="hidden" name="oldtitre"
                                               value="<?php echo $pages[$i]['titre']; ?>"/>

                                        <hr/>

                                        <div id="ckeditorPHP<?php echo $i; ?>"
                                             data-UUID="PHP0006-<?php echo $i; ?>"><?php echo $pages[$i]['content']; ?></div>
                                        <input type="hidden" name="content" id="content-<?php echo $i; ?>" value=""/>
                                        <hr/>

                                        <script> initPost('page<?php echo $i; ?>', 'admin.php?&action=editPage', function (data, other) {
                                                pagesUpdate();
                                            }); </script>
                                        <div class="text-center" style="margin-top:20px;margin-bottom:20px;">
                                            <input type="submit"
                                                   onclick="get('content-<?php echo $i; ?>').value= CK.get(get('ckeditorPHP<?php echo $i; ?>')).getData();sendPost('page<?php echo $i; ?>', null, true);"
                                                   class="btn btn-success w-100" value="Valider les changements"/>
                                            <button type="button"
                                                    onclick="get('content-<?php echo $i; ?>').value= CK.get(get('ckeditorPHP<?php echo $i; ?>')).getData();setShowPopUpPage(get('titre<?php echo $i; ?>').value);sendPost('page<?php echo $i; ?>', null, true);"
                                                    class="btn btn-primary w-100">Prévisualiser
                                            </button>
                                        </div>
                                    </div>
                                <?php } ?>
