...
<?php if (!$_Permission_->verifPerm('PermsPanel', 'social', 'showPage')) {
    ...
    <div class="alert alert-success">
        <strong>Sur cette section, vous pouvez gérer les réseaux sociaux que peuvent rentrer vos membres, et voir leurs
                ...
                <div class="row">
                    <table class="table table-striped table-hover">
                        ...
                        <?php foreach ($donneesSocial as $value) {
                            ...
                            <td>
                                <button type="button" onclick="sendPost('removeSocial&nom=<?= $value['nom']; ?>');"
                                        class="btn btn-danger"><i class="fas fa-trash"></i></button>
                            </td>
                            <script>initPost("removeSocial&nom=<?=$value['nom'];?>", "admin.php?action=removeSocial&nom=<?=$value['nom'];?>", function (data) {
                                    if (data) {
                                        hide('removeSocialnom=<?=$value['nom'];?>');
                                    }
                                });</script>
                            </tr><?php
                        }
        ...
        <div class="col-md-12 col-xl-6 col-12">
            <div class="card   ">
                ...
                <div class="card-footer">
                    <script>initPost("addSocial", "admin.php?action=addSocial", function (data) {
                            if (data) {
                                get('cont-res').innerHTML += '<tr><td>' + getValueByName("addSocial", "nom") + '</td><td></td></tr>';
                                clearAllInput('addSocial');
                            }
                        });</script>
                    <input type="submit" onclick="sendPost('addSocial')" class="btn btn-success w-100"
                           value="Ajouter le réseau social !"/>
                </div>
            </div>
        </div>
        </div>
