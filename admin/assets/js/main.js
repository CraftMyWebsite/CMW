...
function boutiqueUpdate() {
    destroyCK();
    updateCont("admin.php?action=getOffreBoutique", get("allcategorie"), function (data) {
        if (data) {
            initPostCallback();
            ...
            for (var i = 0; i < list.length; ++i) {
                ...
                $('#' + list[i].getAttribute("data-boutique-tagsinput")).tagsinput('add', {
                    'value': parseInt(list[i].getAttribute("data-boutique-tagsinput-value")),
                    'text': list[i].getAttribute("data-boutique-tagsinput-text")
                });
            }
...
function boutiqueActionUpdate(id) {
    updateCont("admin.php?action=getOffreActionBoutique&id=" + id, get("allaction-" + id), function (data) {
        if (data) {
            initPostCallback();
        }
    });
...
function voteUpdate() {
    updateCont("admin.php?action=getLienVote&id=" + idvote, get("all-vote"), function (data) {
        if (data) {
            idvote += 1000;
...
function newsUpdate() {
    destroyCK();
    updateCont("admin.php?action=getNewsList", get("edit-news"), function (data) {
        if (data) {
...
function miniaUpdate() {
    destroyCK();
    updateCont("admin.php?action=getMiniaList", get("allMinia"), function (data) {
        if (data) {
            show('card-minia');
...
function serverUpdate() {
    updateCont("admin.php?action=getServerList", get("modifServer"), function (data) {
        if (data) {
...
function paypalUpdate() {
    updateCont("admin.php?action=getOffrePaypal", get("offrePaypal"), function (data) {
        if (data) {
            initPostCallback(null);
        }
    });
...
function widgetsUpdate() {
    destroyCK();
    updateCont('admin.php?action=getWidgetsList', get('allWidgets'), function (data) {
        if (data) {
...
function pagesUpdate() {
    destroyCK();
    updateCont("admin.php?action=getPagesList", get("allPage"), function (data) {
        if (data) {
            initPostCallback(null);
            initCK();
...
function menuUpdate() {
    updateCont("admin.php?action=getMenuListe", get("allMenu"), function (data) {
        if (data) {
            initPostCallback(null);
        }
    });
...
function gradesUpdate() {
    updateCont("admin.php?action=getGradesList", get("allGrade"), function (data) {
        if (data) {
        ...
        fetchVote("https://serveur-minecraft-vote.fr/api/v1/servers/" + id, function (data, status) {
            ...
            if (isJson(data)) {
                if (parseInt(JSON.parse(data).status) == 1) {
                    ...
                    notif("error", "serveur-minecraft-vote.fr", "Id " + id + " introuvable.");
                }
            } else {
