<script>
    ...
    var allEMail = [ <?php while ($donneesnews = $news->fetch(PDO::FETCH_ASSOC)) {
        echo '"' . $donneesnews['email'] . '",';
    } ?> "null"];
    <?php $news = $bddConnection->query('SELECT * FROM cmw_users WHERE newsletter = 1');?>
    var allSpeudo = [ <?php while ($donneesnews = $news->fetch(PDO::FETCH_ASSOC)) {
        echo '"' . $donneesnews['pseudo'] . '",';
    } ?> "null"];
