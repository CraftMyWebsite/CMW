...
if (isset($_GET['id_topic']) and isset($_GET['choix'])) {
    ...
    if ($choix == 1 && Permission::getInstance()->verifPerm('PermsForum', 'moderation', 'closeTopic')) {
        ...
        if (!isset($_GET['confirmation'])) {
            header('Location: index.php?page=confirmation&id_topic=' . $id . '&choix=2');
        } else {
            if (isset($_POST['reason'])) {
                ...
                if (isset($info_removed['sous_forum']))
                    header('Location: index.php?page=sous_forum_categorie&id=' . $info_removed['id_categorie'] . '&id_sous_forum=' . $info_removed['sous_forum']);
                else
                    header('Location: forum_categorie&id=' . $info_removed['id_categorie']);
            } else
        ...
        //Alors on déplace :P
        if (!isset($_GET['confirmation'])) {
            header('Location: index.php?page=confirmation&id_topic=' . $id . '&choix=3');
